...
try
    ...
    if length(state.init.eom.min) < state.init.eom.numberOfBeams
        state.init.eom.min(state.init.eom.numberOfBeams) = 1;
    end
    if length(state.init.eom.maxPower) < state.init.eom.numberOfBeams
        state.init.eom.maxPower(state.init.eom.numberOfBeams) = 0;
    end
    if length(state.init.eom.maxLimit) < state.init.eom.numberOfBeams
        state.init.eom.maxLimit(state.init.eom.numberOfBeams) = 0;
    end
    if length(state.init.eom.changed) < state.init.eom.numberOfBeams
        state.init.eom.changed(state.init.eom.numberOfBeams) = 1;
    end
    ...
    if isempty(state.init.eom.uncagingMapper.pixels)
        state.init.eom.uncagingMapper.enabled(state.init.eom.numberOfBeams) = 0;
        state.init.eom.uncagingMapper.enabled(:) = 0;
    end
    ...
    updateGUIByGlobal('state.init.eom.uncagingPulseImporter.lineConversionFactor', 'Value', 1000 * state.acq.msPerLine);
...
%     updateGUIByGlobal('state.init.eom.uncagingPulseImporter.pathnameText');
    ...
    state.init.eom.powerBoxNormCoordsString = '[]';
