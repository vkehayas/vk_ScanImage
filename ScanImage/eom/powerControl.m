...
% 
% Last Modified by GUIDE v2.5 11-Nov-2008 18:19:12
%
% Changes:
...
for j = 1 : state.init.maximumNumberOfInputChannels
    ...
    state.init.eom.boxHandles(state.init.eom.beamMenu, j) = rectangle('Position', [(pos(1) + .5) (pos(2) - .5) pos(3) pos(4)], ...
        'FaceColor', 'none', 'EdgeColor', state.init.eom.boxcolors(state.init.eom.beamMenu, :), 'LineWidth', 3, 'Parent', state.internal.axis(j), ...
...
if size(state.init.eom.boxHandles, 1) < state.init.eom.beamMenu & state.init.eom.showBox
    ...
    if state.init.eom.autoSelectFullWidthPowerBox %% & state.init.eom.beamMenu == state.init.eom.scanLaserBeam (VI102008B)
...
if state.init.eom.powerBoxUncagingConstraint & state.init.eom.showBox
    ...
    for j = 1 : state.init.maximumNumberOfInputChannels
        if abs(oldHeight) > 1 %(VI070908A) Only add half-pixel correction when first constraining to line, not subsequent times
            %Added in a half pixel correction (positive in X, negative in Y), to get the box to directly overlap the signal. -- TimO'Connor 6/16/04 - TO061604b
            set(state.init.eom.boxHandles(state.init.eom.beamMenu, j), 'Position', [(pos(1) + .5) (pos(2) - .5) pos(3) pos(4)]);
        else
