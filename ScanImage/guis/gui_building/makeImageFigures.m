...
startImageData = zeros(state.acq.linesPerFrame, state.acq.pixelsPerLine);
...
for i = 1:state.init.maximumNumberOfInputChannels
    state.internal.GraphFigure(i) = figure('Position', figurePosition(i,:) ,'doublebuffer', 'on', 'KeyPressFcn', @genericKeyPressFunction, ... %VI120108A
        ...
        'CloseRequestFcn', 'set(gcf, ''visible'', ''off'')', 'visible', 'off','ColorMap', makeColorMap('gray',8),'ResizeFcn','setImagesToWhole');
    ...
    uimenu(cmenu, 'Label', 'Select ROI', 'Callback', 'global gh; mainControls(''ROI_Callback'',gh.mainControls.ROI)');
    ...
    uimenu(cmenu, 'Label', 'Goto Reset', 'Callback', 'global gh; mainControls(''reset_Callback'',gh.mainControls.reset)');
    uimenu(cmenu, 'Label', 'Set Reset', 'Callback', 'global gh; mainControls(''setReset_Callback'',gh.mainControls.setReset)');
    ...
    state.internal.axis(i) = axes('YDir', 'Reverse','NextPlot', 'add', 'XLim', [1 state.acq.pixelsPerLine],'YLim', [1 state.acq.linesPerFrame], ...
        ...
        'ButtonDownFcn', 'figureButtonOverCallback','UIContextMenu',cmenu);
    if aspectRatio > 0
...
for i = 1:state.init.maximumNumberOfInputChannels
    state.internal.MaxFigure(i) = figure('Position', maxfigurePosition(i,:) ,'doublebuffer', 'on', 'KeyPressFcn', @genericKeyPressFunction, ... %VI120108A
        ...
        'CloseRequestFcn', 'set(gcf, ''visible'', ''off'')','visible', 'off','ColorMap', makeColorMap('gray',8)); 
    
    state.internal.maxaxis(i) = axes('YDir', 'Reverse','NextPlot', 'add', 'XLim', [0 state.acq.pixelsPerLine],'YLim', [0 state.acq.linesPerFrame],...
...
state.internal.MergeFigure = figure('Position',mergePosition,'Name','RGB Channel Merge','Renderer','painters','CloseRequestFcn','set(gcf,''Visible'',''off'');',...
    'DoubleBuffer','on','NumberTitle','off','Tag','MergeFigure','MenuBar','none','Visible','off','ResizeFcn','setImagesToWhole');
state.internal.mergeaxis = axes('Parent',state.internal.MergeFigure,'YDir','reverse','XLim', [1 state.acq.pixelsPerLine],'YLim',[1 state.acq.linesPerFrame],...
