...
% April 2, 2003
%% NOTES
%    Currently, this function is bound to the genericKeyPressFcn of the Main Controls, Standard Mode, ImageGUI, MotorGUI, powerControl, and the image figures
...
%         end
    case 'f1' %load configuration 1  (was 47 '/';VI043008A, VI090908B, VI090908C)
        if all(strcmpi(get(buttonHandles,'Visible'),'on')) 
            loadConfigFast(1);
        end
    case 'f2' %load configuration 2 (was 42 '*';VI043008A, VI090908C)
        if all(strcmpi(get(buttonHandles,'Visible'),'on')) 
            loadConfigFast(2);
        end
    case 'f3' %load Configuration 3 (was 45 '-'; VI043008A, VI090908C)
        if all(strcmpi(get(buttonHandles,'Visible'),'on')) 
            loadConfigFast(3);
        end
    case 'f4' %load configuration 4  %VI090908C
        if all(strcmpi(get(buttonHandles,'Visible'),'on')) 
            loadConfigFast(4);
        end
    case 'f5' %load configuration 5  %VI090908C
        if all(strcmpi(get(buttonHandles,'Visible'),'on')) 
            loadConfigFast(5);
        end
    case 'f6' %load Configuration 6  %VI090908C
        if all(strcmpi(get(buttonHandles,'Visible'),'on')) 
            loadConfigFast(6);
        end
