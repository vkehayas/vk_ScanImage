%Modified: Tim O'Connor 9/26/03 - Allow 'period' delays to apply across loop iterations.
...
%     end
    if state.standardMode.repeatPeriod > .01
        pause(max(0, state.standardMode.repeatPeriod - ...
            state.acq.msPerLine * state.acq.linesPerFrame * state.acq.numberOfFrames));
    end
...
if state.roiCycle.standardPower & state.init.eom.pockelsOn
