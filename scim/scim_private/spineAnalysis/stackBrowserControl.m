            ...
            for m = 1 : length(index)
                ...
                if isfield(metadata, 'units')
                    if metadata.units.unitaryConversions
                        tabularizedData(i, j + 1) = tabularizedData(i, j + 1) + ...
...
try
    ...
    for i = 1 : getLocal(progmanager, hObject, 'nextCorrelationId') - 1
        ...
        for j = 1 : size(persistentData, 1)
            ...
            for n = 1 : expandBy
                ...
                colOffset = 4 + (j-1) * expandBy;%The current data column's index.
    ...
    fprintf(2, 'Failed to export photometry data: %s\n', lasterr);
end
