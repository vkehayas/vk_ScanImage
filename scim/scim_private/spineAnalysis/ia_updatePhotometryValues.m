...
%   VK140929 - Modernized code, renamed loop indicator from 'i' to 'iBounds'
%   VK140929A - Renamed variable 'integral' to 'integralValue' due to conflict with built-in function
%   VK140929D - Swapped places between 'median' and 'mean' as normalizationMethod to match GUI options
%   VK140930A - Only (background-subtracted) spine intensity pixel values above background are now integrated
%
    ...
    if getLocal(progmanager, hObject, 'recalculateNormalization')
        ...
        if optimizeVoxel
            ...
            if length(x) < 100 || length(y) < 100
                for iBounds = 2 : length(x) %VK140929
                    samples = ceil(max(abs(x(iBounds-1)-x(iBounds)), abs(y(iBounds)-y(iBounds-1))));
                    if x(iBounds) ~= x(iBounds - 1)
                        x1(length(x1) + 1 : length(x1) + samples + 1) = x(iBounds - 1) : (x(iBounds) - x(iBounds-1)) / samples : x(iBounds);
                    else
                        x1(length(x1) + 1 : length(x1) + samples + 1) = x(iBounds);
                    end
                    if y(iBounds) ~= y(iBounds - 1)
                        y1(length(y1) + 1 : length(y1) + samples + 1) = y(iBounds - 1) : (y(iBounds) - y(iBounds-1)) / samples : y(iBounds);
                    else
                        y1(length(y1) + 1 : length(y1) + samples + 1) = y(iBounds);
                    end
                end
            else
                x1 = x;
...
%                 normalizationFactor = median(voxel2);
                normalizationFactor = mean(voxel2); %VK140929D
            case 2
%                 normalizationFactor = mean(voxel2);
                normalizationFactor = median(voxel2); %VK140929D
            case 3
...
if ~isempty(integralSelection)
    ...
    integralAboveBackground = integralRegion(integralRegion > background) -background; %VK140930A Only integrate values above background
    integralValue = sum(integralAboveBackground);%VK140929A Renamed variable 'integral' to 'integralValue' due to conflict with built-in function
        %TO090507A - Don't subtract the background here. TO091707A - Don't subtract the minimum pixel value here.
    setLocal(progmanager, hObject, 'intensityIntegral', roundTo(integralValue, 0));
    setLocal(progmanager, hObject, 'integralPixelCount', numel(integralAboveBackground));%TO080507D VK140930A
else
    integralRegion = 0;
    integralValue = 0;
    setLocal(progmanager, hObject, 'intensityIntegral', roundTo(integralValue, 0));
...
%     end
    if background > normalizationFactor || background > integralValue
